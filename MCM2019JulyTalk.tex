...
\begin{frame}<1>[label = Setup]{Solving General Linear Problems Using Series Coefficients\footfullcite{DinEtal20a}}
	...
	\only<1-2>{
		...
		\end{minipage}\\
		\cg &: = \biggl \{ g = \sum_{i=1}^\infty \hg(\vk_i) v_{\vk_i} : \norm[\cg]{g} := \bignorm[\tau]{\hg}\biggr \}, \qquad v_{\vk} = \sol(u_{\vk}), \ \tau \le \rho \\
		}
...
\end{equation*}
    ...
    & \alert<4>{= \max \bigl \{ n^*(f,\varepsilon) : f \in \cc_{\vlambda, n_1, A} \cap \cb_R \bigr\}} \\
    ...
    \end{align*}
    \uncover<5>{$\alg$ is \alert{essentially optimal}}
    }
    \only<6>{For $\rho = \tau$ ($\rho'=\infty$), the problem $\sol_d :\cc_{d,\vlambda_d, n_1, A} \to \cg_d$ is 
    \begin{description}
 ...
 $
  \sup_{d\in\naturals} \, \exp(-cd) \sum_{i=1}^\infty \exp\left(-c\left(\frac{1}{\lambda_{d,\vk_i}}\right)\right) <\infty$ \; for all $c>0$
    ...
    \end{description}
    Analogous results exist for $\rho > \tau$ ($\rho' < \infty$)
    }
