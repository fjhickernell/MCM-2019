...
\begin{itemize}
	\item $\sol : \cf \to \cg$ is linear \alert{solution} operator, $\cf$ is space of functions defined on $\Omega \subseteq \reals^d$ \\
	\qquad e.g., $\sol(f) = f$, \quad $- \nabla^2 \sol(f) = f,   \sol(f) = 0$  on boundary
	\item $\app(f,n) = \sum_{i=1}^n L_i(f) g_i$ is \alert{approximation} \\
...
\uncover<2->{\item \alert{Solvability:} what assumptions on $f$ to determine $n^*(f,\varepsilon) \in \naturals$ \alert{correctly}?  
	\only<4->{\smallscoop\only<5->{\hspace{-6ex}\raisebox{-1.5ex}{\color{red}\fontsize{40}{48}\selectfont $\times$}} or \smallcone}
\only<3>{
	
	\begin{itemize}
	\item $n^*(f,\varepsilon) = \min\{n : \norm[\cf \to \cg]{\sol - \app(\cdot,n)} \le \varepsilon / R\}$  if $f \in \smallscoop$ of radius $R$
	
	\item Alternative is to assume $f \in \smallcone$ and to bound $ \norm[\cf]{f}$ \footfullcite{HicEtal17a,KunEtal19a,DinEtal20a}.  \\
	\emph{What you do not see is not much worse than what you see?}
\end{itemize}}

\only<4->{\item \alert{Optimality:} is $n^*(f,\varepsilon)$ essentially as small as possible?
...
\begin{frame}{Solving General Linear Problems Using Series Coefficients, $\LambdaSer$\footfullcite{DinHic20a}}
	...
	\only<1>{\cf &:= \left \{ f = \sum_{i=1}^\infty \hf(\vk_i) u_{\vk_i} : \norm[\cf]{f} := \norm[\rho]{\left(\frac{\bigabs{\hf(\vk_i)}}{\lambda_{\vk_i}} \right)_{i =1}^\infty} \right \}, \qquad 
		...
		\cg &: = \biggl \{ g = \sum_{i=1}^\infty \hg(\vk) v_{\vk} : \norm[\cg]{g} := \bignorm[\tau]{\hg}\biggr \}, \qquad v_{\vk} = S(u_{\vk}) \\}
	\cc &: = \left\{f \in \cf :  
	\sigma_\ell(f) \le a b^{\ell - k}\sigma_k(f) \quad \forall k \le \ell \right\}  \\
	\app(f,n) &= \sum_{i=1}^{n} \hf(\vk_i) v_{\vk_i} \text{ is optimal for fixed }n, \qquad \norm[\cg]{S(f) - \Sapp(f,n_k)} \le \frac{ab \sigma_k(f)}{\sqrt{1 - b^2}} \\[-1ex]
...
\begin{frame}
	...
	\quad \omega_{i} \downarrow
		...
		\vspace{-2ex}
		\begin{multline*}
		\cc'_{\vlambda, \vomega, A'} : = \{ f \in \cf : \norm[\cf]{f} \le A'\norm[\cf']{f}\} \subseteq \cc_{\vlambda, n_1, A} : = \biggl\{ f \in \cf : \norm[\cf]{f} \le A\norm[\rho]{\biggl ( \frac{\hf(\vk_i) }{\lambda_{\vk_i}} \biggr) _{i=1}^{n_1}} \biggr\} \\
